...
def generate_frames():
    ...
    while True:
        ...
        if reference_frame is not None:
            ...
            if desc is not None and len(kp) > 10:
                ...
                if len(good) > 8:
                    ...
                    if H is not None:
                        live_h = H @ np.array([[ref_center[0]],
                                               [ref_center[1]],
                                               [1.0]])
        ...
        if live_pt is not None:
            ...
            cv2.arrowedLine(display, live_pt, ref_center, arrow_color, 3, tipLength=0.3)
...
document.addEventListener("keydown", function(event) {
    if (event.key === "s") sendCommand("s");
    if (event.key === "r") sendCommand("r");
    if (event.key === "q") sendCommand("q");
});
...
<div class="container">
    ...
    <div class="button-row">
        <button onclick="sendCommand('s')">Set Reference (S)</button>
        <button onclick="sendCommand('r')">Reset (R)</button>
        <button onclick="sendCommand('q')">Quit (Q)</button>
    </div>
</div>
