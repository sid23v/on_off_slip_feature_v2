...
# ======= NEW: feature-rich gating variables & thresholds =======
...
# ===== NEW: semantic test for "feature-rich" region =====
...
while True:
    ...
    # ----- TRACKING PATH (kept intact) -----
    if reference_frame is not None:
        kp, desc = compute_keypoints_descriptors(gray)

        # ===== NEW: if we had a feature-rich reference and now we're over a feature-poor region -> immediate reset
        try:
            ...
            if was_feature_rich and not curr_feature_rich:
                # abrupt move from circle -> white (feature rich -> poor)
                print("Auto-reset (left feature-rich region - abrupt move).")
                reset_tracking()
                continue
        except Exception:
            # conservative: if semantic check fails, do nothing special and allow existing guards to handle it
            pass
        # ======================================================================
    ...
    # ===== NEW: block setting reference when hovering over feature-poor region =====
    if key == ord('s'):
        # Compute keypoints on the current frame to decide if reference can be set
        kp_ref, desc_ref = compute_keypoints_descriptors(gray)
        try:
            if not is_feature_rich(gray, kp_ref):
                # Don't allow setting reference over white/feature-poor regions
                print("Cannot set reference: insufficient features / low texture in current view.")
                # flash red overlay to notify user briefly
                overlay_counter = OVERLAY_FRAMES
                # do not set reference
                continue
        except Exception:
            # If semantic check fails for any reason, be conservative and block the set
            print("Cannot set reference: semantic validity check failed.")
            overlay_counter = OVERLAY_FRAMES
            continue

        # If passed semantic gating, set the reference exactly as before
        ...
        last_scale_est = 1.0
        was_feature_rich = True          # mark that reference was set from a feature-rich region
